...
implementation
  ...
  const
  DllAdi='MathLibrary.dll';//Kullanýlacak dll adý
...
//-- Class Fonksiyonlar Burada Tanýmlanýyor
  ...
  begin
     ...
     if DllHandle=0 {dll bulunamadý} then //ShowMessage('Dll Dosyasý Bulunamadý')
      else
        try
          Address:=GetProcAddress(DllHandle,FuncOpenCam);//dll içindeki fonksiyon adresini bul
          if not Assigned(Address) {dll içindeki adres bulunamadý} then
             // ShowMessage('Dll Ýçinde Aranan Fonksiyon Bulunamadý !')
          else begin
          ...
          //ShowMessage('Fonksiyon iþlenirken HATA!');
          end;
        end;
